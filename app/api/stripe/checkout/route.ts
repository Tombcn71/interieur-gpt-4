...
export async function POST(req: NextRequest) {
  ...
  if (!process.env.NEXT_PUBLIC_APP_URL) {
    ...
    return NextResponse.json(
      { error: "NEXT_PUBLIC_APP_URL is not configured. Please check your environment variables." },
      { status: 500 },
    )
  }
  ...
  try {
    ...
    if (!priceId) {
      ...
      return NextResponse.json({ error: "Geen prijs ID opgegeven" }, { status: 400 })
    }

    console.log(`Creating checkout session for user ${session.user.id} with priceId ${priceId}`)
